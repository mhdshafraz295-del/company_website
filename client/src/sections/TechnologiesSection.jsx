          ...
          {techCategories.map((group, idx) => {
            ...
            return (
              <Reveal3D key={group.category} delay={idx * 0.02}>
                <TiltCard className="bg-white/80 dark:bg-[#0D1322]/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 shadow-xl shadow-slate-200/40 dark:shadow-slate-950/40 rounded-3xl p-6 sm:p-7 space-y-4 hover:shadow-2xl hover:border-cyan-300 dark:hover:border-cyan-500/50 flex flex-col justify-between h-full">
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3">
                      <div className={`p-2.5 rounded-2xl border ${group.color}`}>
                        <Icon className="w-5 h-5" />
                      </div>
                      <h3 className="text-base font-bold text-slate-900 dark:text-white">{group.category}</h3>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {group.skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-cyan-700 dark:hover:text-cyan-400 hover:bg-cyan-50 dark:hover:bg-slate-800 hover:border-cyan-200 transition-colors shadow-sm"
                        >
                          {skill}
                        </span>
                      ))}
