...
export default function AdminTeamPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim() || !formData.position.trim()) {
      setErrorMessage('Name and Position are required fields.');
      return;
    }
    ...
    try {
      ...
      if (publicContext?.refetchTeam) publicContext.refetchTeam();
    } catch (err) {
      setErrorMessage(err.response?.data?.message || 'Failed to save team member.');
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-200 select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gradient-to-r from-[#0d1322] via-[#0f172a] to-[#0b101d] border border-slate-800/80 rounded-2xl p-6 shadow-xl">
        <div className="space-y-1">
          <div className="inline-flex items-center space-x-2 text-violet-400 text-xs font-semibold uppercase tracking-wider">
            ...
            <span>Personnel & Engineering Profiles</span>
          </div>
          <h1 className="text-2xl font-extrabold text-white">Team CMS</h1>
          <p className="text-xs text-slate-400">
            Configure active company team members, positions, photos, and social links.
          </p>
        </div>
        <button
          onClick={handleOpenAdd}
          className="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-bold py-2.5 px-4 rounded-xl shadow-lg flex items-center space-x-2 w-fit transition active:scale-95"
        >
      ...
      <div className="flex items-center justify-between bg-[#0d1322]/80 border border-slate-800/80 rounded-xl p-4">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search team member name, position..."
            value={search}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16 space-y-3 bg-[#0d1322]/60 border border-slate-800/80 rounded-2xl">
          ...
          <p className="text-xs text-slate-400">Loading team profiles...</p>
        </div>
      ) : filtered.length > 0 ? (
                  ...
                  <tr key={member.id} className="hover:bg-slate-900/40 transition">
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        {member.photo ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-violet-950 text-violet-300 font-extrabold flex items-center justify-center border border-violet-800 shrink-0">
                            {member.name.charAt(0)}
                          </div>
                        )}
                        <div>
                          <div className="font-bold text-white text-sm">{member.name}</div>
                          <div className="text-[11px] text-slate-500 font-mono">Order: {member.displayOrder}</div>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-4 text-cyan-400 font-semibold">{member.position}</td>
                    <td className="py-4 px-4">
                      {member.isActive ? (
                    ...
                    <td className="py-4 px-4 text-right space-x-2">
                      <button
                        ...
                        className="p-2 bg-slate-800 hover:bg-slate-700 text-violet-400 rounded-xl transition"
                        title="Edit Member"
                      >
        ...
        <div className="text-center py-16 bg-[#0d1322]/80 border border-slate-800 rounded-2xl space-y-3">
          ...
          <p className="text-xs text-slate-400">Click "Add Team Member" above to create profiles.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#0b101d] border border-slate-800 rounded-3xl w-full max-w-xl max-h-[92dvh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-[#0e1626]/90 backdrop-blur shrink-0">
              <div className="flex items-center space-x-3">
                <div className="w-9 h-9 rounded-xl bg-violet-950/70 border border-violet-800/60 text-violet-400 flex items-center justify-center">
                  <Users className="w-5 h-5" />
                </div>
                <div>
                  <h2 className="text-base font-bold text-white">
                    {editingMember ? 'Edit Team Profile' : 'Add Team Member'}
                  </h2>
                  <p className="text-[11px] text-slate-400">
                    Configure staff member credentials, job title, and social links.
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              ...
              <form id="team-form" onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-[#070b14] border border-slate-700/80 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-violet-500"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-300">Position / Title *</label>
                    <input
                      type="text"
                      required
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    rows={2}
                    placeholder="Short bio for team card..."
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      type="email"
                      placeholder="david@nexgen.local"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-[#070b14] border border-slate-700/80 rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-violet-500 font-mono"
                    />
                  </div>
                  <div className="space-y-1 flex flex-col justify-end">
                    <label className="flex items-center space-x-2 cursor-pointer pb-2">
                      ...
                      />
                      <span className="text-xs font-semibold text-slate-200">Active Team Member</span>
                    </label>
                  </div>
                </div>
              </form>
            ...
            <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t border-slate-800 bg-[#0e1626]/90 backdrop-blur shrink-0">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-xs font-bold text-white rounded-xl shadow-lg transition flex items-center space-x-2 disabled:opacity-50"
              >
